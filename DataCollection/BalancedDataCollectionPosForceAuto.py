...
# startHeight = 78 + OFFSET
# heights = [89, 88, 87]
# heights = [90, 85, 80]
# heights = [88 + OFFSET]
heights = [82, 81, 80, 79, 78]
...
if __name__ == "__main__":
    ...
    # Save a bunch of zero data
    if saveData:
        print("Saving zero data")
        # side = 0
        for i in range(40000 - 5000):
 ...
 # Home the printer
    ...
    while True:
            ...
            if userInput == 'w':
                ...
                print("Invalid input")
            PrinterController.move_axis_absolute('x', x, 800)
            PrinterController.move_axis_absolute('y', y, 800)
            time.sleep(1)

    print('Original start x: ', startX)
    print('Original start y: ', startY)

    StartX = x
    StartY = y

    print('New start x: ', StartX)
    print('New start y: ', StartY)
    input()
    ...
    # Make grid centers only to 2 decimal places
    grid_centers = [(round(x, 2), round(y, 2)) for (x, y) in grid_centers]

    print(f"Grid centers: {grid_centers}")
    input()

    # Add the startX and startY to the grid points
    grid_centers = [(xGrid + x, yGrid + y) for (xGrid, yGrid) in grid_centers]
    print(f"Grid centers: {grid_centers}")
    input()
    ...
    # Pass through all the grid points for visual inspection
    for (xTest, yTest) in grid_centers:
        oldX = x
        oldY = y
        x = xTest
        y = yTest
        PrinterController.move_axis_absolute('x', xTest, 800, oldX)
        PrinterController.move_axis_absolute('y', yTest, 800, oldY)
        # time.sleep(1)

    print("Is the grid correct? Press Enter to continue")
    input()
