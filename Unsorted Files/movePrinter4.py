...
try:
    ...
    while y_travel < 68:
        ...
        while x_travel < 68:
            time.sleep(1)
            # Move z axis down 5mm 
            # send_gcode_command("G1 Z-5 F300")
            # while is_printer_busy():
            #     time.sleep(0.1)
            # # Move z axis up 5mm
            # send_gcode_command("G1 Z5 F300")
            # while is_printer_busy():
            #     time.sleep(0.1)
            # Move 1mm to the right in the X direction 
            # CHAR for step 
            send_gcode_command(f"G1 X{step} F{feed_rate}")
            # send_gcode_command(f"G1 X1 F{feed_rate}")
            while is_printer_busy():
        ...
        send_gcode_command(f"G1 Y{step} F{feed_rate}")
        while is_printer_busy():
            time.sleep(0.1)
