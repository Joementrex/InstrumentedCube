            ...
            if data!= []:
                ...
                if Visualier == True:
                    ...
                    if pred_face == 0 or pred_pos == 0:
                        ...
                        Vis.change_square_color('None', 0,0)
                    elif pred_face == 1:
                        Vis.colour_face('Top')
                        Vis.change_square_color('Top', pred_pos, pred_pos2)
                    elif pred_face == 2:
                        Vis.colour_face('Front')
                        Vis.change_square_color('Front', pred_pos, pred_pos2)
                    elif pred_face == 3:
                        Vis.colour_face('Left')
                        Vis.change_square_color('Left', pred_pos, pred_pos2)
                    elif pred_face == 4:
                        Vis.colour_face('Right')
                        Vis.change_square_color('Right', pred_pos, pred_pos2)
                    elif pred_face == 5:
                        Vis.colour_face('Back')
                        Vis.change_square_color('Back', pred_pos, pred_pos2)
                    # Vis.change_square_color(pred_face, pred_pos)

            
if __name__ == '__main__':
    classifier = MultiPosDetector()
