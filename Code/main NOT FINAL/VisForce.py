...
def draw_squares():
    ...
    for i, centers in enumerate(face_centers):
        ...
        for center in centers:
            ...
            # Get square colour 
            intensity = SQUARE_COLORS[i * 4 + centers.index(center)]
            intensity = intensity[0]


            if i == 0 or i == 1:
                z = 0.01 + (intensity * 0.1)
            else:
                z = -0.01 - (intensity * 0.1)
...
def change_square_color(face, square_index, force=0):
    ...
    if 0 <= face_index < 6 and 0 <= square_index < 4:
        ...
        for i in range(6):
            for j in range(4):
                SQUARE_COLORS[i * 4 + j] = BLUE
        forceFloat = force / 3
        SQUARE_COLORS[face_index * 4 + square_index] = [forceFloat, 0.0, 0.0]
