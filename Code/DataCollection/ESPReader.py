...
def get_esp_data(ignoreErrors=False):
    ...
    if device == 'reciever':
        ...
        # Error check the data, if 3 values in a row are zero then return []
        if not ignoreErrors:
            for i in range(0, len(data), 3):
                if data[i] == 0 and data[i+1] == 0 and data[i+2] == 0:
                    print('Broken sensor reading zero values, check wiring')
                    print(data)
                    time.sleep(1)
                    return []
...
# Backup function to read the values off the master device 
def read_master():
    # if ser2 is None:
    #     ser2 = serial.Serial(com2, baud_rate)
    data = []
    if ser2.in_waiting > 0:
        ...
        # if data is [] then redo the process
        if data == []:
            return read_master()
    return data
...
# Read each line of data from the ESP32
def read_raw():
    while True:
        data = ser.readline().decode('utf-8').strip()
        print(data)
...
if __name__ == "__main__":
    # Read the raw data from the ESP32
    read_raw()